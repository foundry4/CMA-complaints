...
module.exports = (data) => {
    ...
    var product_list = [];
        ...
        for (index in products){
            ...
            if(desc!=""){
                var row = Object.keys(product_section).map(function (key) {
                    var ref = label + '_' + product_section[key].name;
                    var val = data[ref];
                    return {name:product_section[key].text, category: text, value:val, url:'/which_products'}
                });
                product_list = product_list.concat(row)
            }
        }
