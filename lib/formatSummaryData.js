...
// console.log(data['report_reason'], data['report_reason'].indexOf('pricing'),data['product'],data['other_product'])
        ...
        if(data&&data['medium']!==undefined) {
            business = Object.keys(business_section).map(function (key) {
                ...
                if (key === 'medium') {
                    if (val === 'online') val = 'Online';
                    if (val === 'store') val = 'In store';
                    if (val === 'telephone') val = 'On the telephone';
                    if (val === 'other') val = data['other_location'];
                }
                if (key === 'business-name' && (data['medium'] === 'online')) {
                    url = '/what_is_business_url';
                }
                if (key === 'business-name' && (data['medium'] === 'telephone')) {
                    url = '/what_is_business_phone';
                }
                if (key === 'location') {
                    if (data['medium'] === 'online') {
                        val = data['website'];
                        if (data['business-email']) val += ', ' + data['business-email'];
                        url = '/what_is_business_url';
                    }
                    else if (data['medium'] === 'telephone') {
                        val = data['telephone'];
                        url = '/what_is_business_phone';
                    } 
                    else {
        ...
        if (data && data['more-info']) {
            ...
            if (data['more-info'] === 'true') {
                if(data['contact-email']||data['contact-number']){
                    ...
                    console.log('contact details missing error')
                    total_errors++;
                    errors.push({
        ...
        let show_products;
        if(data) {
            show_products = data && data['report_reason']&&data['report_reason'].indexOf('pricing') >-1
        }
        console.log('after summary');
        return { business, reason, product_list, contacts,other_product, total_errors, errors, missingProducts, show_products,error_end, error_start };
